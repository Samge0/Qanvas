...
- **可用内存 ≥ 6 GB 是硬性底线，整机内存建议 12 GB+。** 运行时要求生成启动瞬间的
  *当前可用内存*（MemAvailable）≥ 6 GB——与尺寸/步数设置无关，因此 16 GB 手机
  后台占用高时同样可能失败。**设置页顶部有内存检测卡，请先检测再下载 10 GB 模型。**
- 约 11 GB 可用存储，用于模型一次性下载（约 10.3 GB，断点续传 + 校验和验证；
  支持 huggingface.co 与国内镜像 hf-mirror.com 双下载源）
